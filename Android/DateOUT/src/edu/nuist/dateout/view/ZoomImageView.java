...
/**
 *个性的图片预览与多点触控（自由移动，自由缩放，双击放大缩小） 
 */
public class ZoomImageView extends ImageView implements OnGlobalLayoutListener, OnLongClickListener,OnClickListener,OnScaleGestureListener, OnTouchListener {
...
//					
					//缓慢放大
					postDelayed(new AutoScaleRunnable(mMidScale, x, y), 1);
					isAutoScale=true;
				}else {
//					mScaleMatrix.postScale(mInitScale/getScale(), mInitScale/getScale(),x,y);
//					setImageMatrix(mScaleMatrix);
					
					postDelayed(new AutoScaleRunnable(mInitScale, x, y), 1);
					isAutoScale=true;
				}
